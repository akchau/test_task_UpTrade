...
# Реализация
Дерево-меню реализовано с помощью связанного списка и рекурсивного вызова шаблона `tree.html` в случае нахождения активных родительских категорий текущего уровня. Это позволяет делать один запрос к БД при загрузке страницы.
...
- `Section`. - таблица с инфромацией о секциях меню. `name` - аналогично `Menu`, человекочитаемое имя секции которое будет отображаться. `adress` - адресс секции.
